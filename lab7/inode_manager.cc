...
inode_manager::inode_manager()
{
  ...
  uint32_t root_dir = alloc_inode(extent_protocol::T_DIR, mode, user_id);
  if (root_dir != 1) {
 ...
 * Return its inum. */
uint32_t
inode_manager::alloc_inode(uint32_t type, unsigned int mode, unsigned short user_id)
{
  /* 
   * your lab1 code goes here.
  ...
  ino_disk->mode = mode;
  ino_disk->uid = user_id;
  ino_disk->gid = user_id;
  //printf("%o %d", mode, ino_disk->uid);
